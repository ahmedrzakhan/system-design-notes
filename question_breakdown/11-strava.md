...
# Strava System Design - Last Minute Revision Points

## 🎯 Core Problem & Requirements

• **What**: Fitness tracking app for running/cycling with social features
• **Key Functions**: Start/pause/stop activities, real-time tracking, view friends' activities
• **Scale**: 10M concurrent activities
• **Critical**: High availability > consistency, offline support, low latency

## 🏗️ Architecture Foundation

• **Client-heavy design**: GPS tracking happens locally (reduces server load 100x)
• **Offline-first**: Buffer data in memory → persist every 10s → sync when online
• **State management**: Activities have states (STARTED, PAUSED, RESUMED, COMPLETE)

## 📊 Data Models

• **User**: id, name, email, country
• **Activity**: id, userId, type, startTime, status, distance
• **Route**: activityId, lat/lng, timestamp
• **Friend**: userId, friendId relationship

## 🔌 Key APIs

• `POST /activities` - Create new activity
• `PATCH /activities/:id` - Update state (start/pause/complete)
• `POST /activities/:id/routes` - Add GPS points (bulk on completion)
• `GET /activities?mode=FRIENDS` - Social feed

## ⚡ Scaling Strategies

• **Database sharding**: By completion time (recent = hot, old = cold storage)
• **Horizontal scaling**: Stateless activity servers behind load balancer
• **Caching**: Redis for active activities and leaderboards
• **Data tiering**: Hot (recent) → Warm (3-12mo) → Cold (>1yr archived)

## 🏆 Leaderboard Deep Dive

• **❌ Bad**: Real-time SQL aggregation across millions of records
• **✅ Good**: Daily batch job updating aggregated leaderboard table
• **✅✅ Best**: Redis Sorted Sets - `ZADD leaderboard:run:global user_id distance`
• **Time filtering**: Store activity timestamps in sorted sets + hash maps

## 📱 Offline Support Details

• **GPS buffering**: In-memory → persistent storage every 10s
• **Max data loss**: 10 seconds on crash
• **Sync strategy**: Bulk upload route data on completion or connectivity
• **Storage**: Core Data (iOS) / Room (Android)

## 🔄 Real-time Features

• **Live tracking**: Friends can see ongoing activities
• **Update frequency**: GPS every 2-5s, server sync every 10s
• **Implementation**: Polling > WebSockets (predictable updates, simpler)
• **UX trick**: Lag display 5-10s for smooth animations

## 🔋 Battery Optimization

• **Adaptive GPS**: Running 5s intervals, cycling 2s, stationary reduces frequency
• **Batch network calls**: Minimize radio wake-ups
• **Smart persistence**: Balance data safety vs battery drain

## 📈 Storage Calculations

• **Volume**: 100M DAU × 1 activity/day = 36.5B activities/year
• **Size**: ~15KB per activity (including route data)
• **Total**: ~547TB/year → Need tiered storage strategy

## 🛠️ Technology Stack

• **Client**: GPS sensors + local processing
• **Backend**: RESTful APIs, microservices architecture
• **Database**: Sharded SQL (activities) + Redis (cache/leaderboards)
• **Storage**: Hot/warm databases + cold storage (S3)

## 🎯 Interview Flow Strategy

1. **Requirements** (5-10min): Clarify scope, scale, key features
2. **High-level design** (10-15min): Client-server flow, offline approach
3. **Deep dives** (20-30min): Pick 2-3 areas (scaling, leaderboards, offline)
4. **Wrap-up** (5-10min): Monitoring, follow-ups, edge cases

## 🚨 Common Follow-up Questions

• **GPS drift/accuracy**: Filtering algorithms, snap-to-roads, Haversine formula
• **Cheating detection**: Velocity checks, ML anomaly detection
• **Wearables integration**: Sync through phone, similar architecture
• **Multi-sport activities**: State machine for sport transitions
• **Privacy**: Hide start/end locations, private activities

## 🎪 Advanced Features to Mention

• **Segments**: Popular route sections with leaderboards
• **Live segments**: Real-time comparison during activity
• **Heatmaps**: Aggregate popular routes
• **Route planning**: Pre-planned routes with elevation

## 📊 Key Metrics

• **Performance**: API latency, GPS accuracy, battery usage
• **Scale**: Concurrent activities, storage growth
• **Business**: DAU, activities per user, social engagement

## 💡 Key Insights to Emphasize

• **Modern phones are powerful** - leverage client-side processing
• **Offline capability is non-negotiable** for fitness apps
• **Battery optimization** drives many architectural decisions
• **Redis sorted sets** are perfect for leaderboards
• **Data doesn't need to be hot forever** - tier by access patterns
